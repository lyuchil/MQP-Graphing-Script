...
for policy_folder, data_points in all_data_points.items():
    ...
    for setting, mean, confidence in data_points:
        plt.errorbar(setting, mean, yerr=confidence, fmt='o', color=color, label=policy_folder, ecolor='black')
    ...
    plt.scatter(x_values, y_values, color=color, alpha=0.1)
...
plt.legend(handles=[legend_handles[policy_folder][0] for policy_folder in policy_folders], title='Policy', bbox_to_anchor=(1.02, 0.5), loc='upper left')


plt.xlabel('Jitter Settings')
